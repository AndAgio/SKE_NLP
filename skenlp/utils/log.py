...
def get_logger(arguments, mode='smart'):
    if mode == 'smart':
        logger_name = '{}_over_{}_and_{}'.format(arguments.model, arguments.train_dataset, arguments.test_dataset)
        my_logger = SmartLogger(logger_name, verbose=False, log_dir=arguments.log_folder)
...
class SmartLogger(logging.getLoggerClass()):
    ...
    def get_log_file(self):
        with open('{}/ids_file.txt'.format(self.log_dir), 'r') as f:
            last_id = f.readlines()[-1].strip()
        return "{}/info_log_{}.log".format(self.log_dir, last_id)
...
def test_verbose():
    ...
    verbose_log = SmartLogger('verbose', verbose=True, log_dir='logs')
    ...
    verbose_log.print_it('We now log everywhere irrespective of verbosity')


def test_quiet():
    print('*' * 80 + '\nQuiet logging (stdout: only FRAMEWORK + file: all levels)\n' + '*' * 80)
    quiet_log = SmartLogger('quiet', verbose=False, log_dir='logs')
    quiet_log.warning('We now log only to a file log')
    quiet_log.print_it('We now log everywhere irrespective of verbosity')
