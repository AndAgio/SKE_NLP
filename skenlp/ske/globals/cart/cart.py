...
from ..schema import Value, LessThan, GreaterThan
...
class CartPredictor:
    ...
    def __init__(self, predictor: Union[DecisionTreeClassifier, DecisionTreeRegressor] = DecisionTreeClassifier(), normalization=None):
        self._predictor = predictor
        self.normalization = normalization
        self.valuable_tokens = None

    def set_valuable_tokens(self, valuable_tokens):
        self.valuable_tokens = valuable_tokens

    def get_valuable_tokens(self):
        return self.valuable_tokens
    ...
    def __get_constraints(self, nodes: Iterable[(int, bool)]) -> LeafConstraints:
        ...
        if self.normalization is not None:
            thresholds = [threshold * self.normalization[feature][1] + self.normalization[feature][0] for feature, threshold in zip(features, thresholds)]
