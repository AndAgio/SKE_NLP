...
from ..schema import Value, LessThan, GreaterThan, Between, Constant, term_to_value, Outside, DiscreteFeature
from skenlp.utils import get_int_precision
...
def readable_clause(clause: Clause, new_line: bool = True) -> str:
    formatter = TermFormatter.prettyExpressions(True, RULES_OPERATORS)
    if clause.is_fact:
        return f"{clause.head.args[-1]}"
    elif clause.is_directive:
        redable_body = format_body_terms(clause.body, formatter)
        return ":- " + redable_body
    else:
        redable_body = format_body_terms(clause.body, formatter)
        return f"{clause.head.args[-1]} :-\n    {redable_body}"
...
def format_body_terms(body, formatter: TermFormatter) -> str:
    body_str = str(formatter.format(body))
    # print('\nbody_str: {}'.format(body_str))
    body_splits = body_str.split(',')
    # print('body_splits: {}'.format(body_splits))
    return ', '.join([format_body_term(term) for term in body_splits])


def format_body_term(term: str) -> str:
    # print('term: {}'.format(term))
    splits = re.split('>= |=< |< |>', term)
    # print('splits: {}'.format(splits))
    element = splits[0].lstrip().rstrip()
    value = float(splits[-1])
    operand = term.lstrip(splits[0]).rstrip(splits[-1]).lstrip().rstrip()
    ...
    if value == 0:
        if operand == '>=':
            raise ValueError('Something went wrong with rule extractor!')
        elif operand in ['>']:
            return element.lower().strip('`')
        else:
            return '¬ {}'.format(element.lower().strip('`'))
    elif value == 1:
        if operand == '=<':
            raise ValueError('Something went wrong with rule extractor!')
        elif operand in ['<']:
...
def readable_theory(theory: Theory, new_line: bool = True) -> str:
    if len(str(theory)) == 0:
        return ""
    else:
        clauses = [readable_clause(clause, new_line) for clause in theory]
        return ".\n".join(clauses) + "."
