...
class HessianLocalExplainer(BaseLocalExplainer):
    def __init__(self,
                 tokenizer,
                 model,
                 multi_label=False,
        ...
        self.explainer_name = 'hess'
        self.explainer = EmbeddingExplainerTorch(self.prediction_func)

    def prediction_func(self, input_embedding):
        attention_mask = torch.ones(input_embedding.shape[:2]).to(self.device)
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.float()
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
        all_encoder_layers, all_encoder_attention_scores = self.model.bert.encoder(input_embedding,
                                                                                   extended_attention_mask)
        sequence_output = all_encoder_layers[-1]
        pooled_output = self.model.bert.pooler(sequence_output, optional_attn_mask=attention_mask)
    ...
    def explain(self, sentence, label=None, sentence_id=0, dataset_config=None,
        ...
        sentence_embedding = self.model.bert.embeddings(torch.tensor(tokens['input_ids']).to(self.device))
        embedding_shape = torch.tensor(tokens['input_ids']).shape
        baseline_embedding = self.model.bert.embeddings(torch.tensor(np.zeros(embedding_shape,
                                                                              dtype=np.int64)).to(self.device))
        hessian_values = self.explainer.attributions(inputs=sentence_embedding,
                                                     baseline=baseline_embedding)
        hessian_values = torch.transpose(torch.squeeze(torch.tensor(hessian_values)), 0, 1)
        words, hessian_values = self.refine_tokens_and_scores(tokens, hessian_values)
        assert len(words) == hessian_values.shape[0]
        if verbose:
            self.logger.print_it('hessian_values: {}'.format(hessian_values))
        if visualize:
            self.visualize(scores=hessian_values,
                           outputs=outputs,
        ...
        if get_pred_labels:
            return words, hessian_values, pred_labels
        else:
            return words, hessian_values
...
class EmbeddingExplainerTorch(object):
    ...
    def accumulation_function(self,
        ...
        if interaction_index is not None:
            batch_hessian = torch.autograd.grad(
            ...
            for feature in range(batch_input.size(1)):
                batch_hessian[:, feature, :, :] = torch.autograd.grad(
